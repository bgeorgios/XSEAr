...
Function to fit a Tapered Pareto (TP) distribution using the Maximum Likelihood (ML) estimation method. An initial guess for both the lower truncation threshold (beta) and the upper-cutoff (theta) parameters is required since they are numerically optimized.
